 ...
 *
 * By default, all floating-point calculations are performed using `vqf_real_t`. Set the `VQF_SINGLE_PRECISION` define to
 * change this type to `float`. Note that the Butterworth filter implementation will always use vqf_real_t precision as
 * using floats can cause numeric issues.
 */
#ifndef VQF_SINGLE_PRECISION
typedef vqf_real_t vqf_real_t;
#else
...
public:
     ...
     *
     *     int N = 1000;
     *     vqf_real_t gyr[N*3]; // fill with gyroscope measurements in rad/s
     *     vqf_real_t acc[N*3]; // fill will accelerometer measurements in m/s²
     *     vqf_real_t mag[N*3]; // fill with magnetometer measurements (arbitrary units)
     *     vqf_real_t quat9D[N*4]; // output buffer
     *
     *     VQF vqf(0.01); // 0.01 s sampling time, i.e. 100 Hz
     ...
     */
    static void filterAdaptStateForCoeffChange(vqf_real_t last_y[], size_t N, const vqf_real_t b_old[3],
     ...
     */
    static void filterVec(const vqf_real_t x[], size_t N, vqf_real_t tau, vqf_real_t Ts, const vqf_real_t b[3],
